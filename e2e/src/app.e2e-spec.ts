...
describe('workspace-project App', () => {
  ...
  it('buttons from mobile should not exist, button from high width page should exist, and should return correct text', () => {
    ...
    expect(AppPage.getTextTrendsBtn()).toEqual(`Ostatnie trendy`);
  ...
  it('error and warning should display', () => {
    browser.sleep(7000);
    page.navigateToFullWidth();
    expect(element(by.id('premiere')).isPresent()).toEqual(true);
    ...
    page.getSubmitBtn().click().then(() => {
      browser.wait(() => {
        return element(by.id('displayView')).isPresent();
      });
    });
  });
