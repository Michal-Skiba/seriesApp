...
export class SeriesService {
  ...
  limitReq: number = 39;
  readonly limiter = new RequestLimiter(10000, 40);
  ...
  searchSeries(seriesTitle: string, page: number): Observable<SearchData> {
    return this.limiter.limit(this.http.get<SearchData>(`${environment.apiUrl}search/tv?api_key=${environment.apiKey}&language=${this.language}-US&query=${seriesTitle}&page=${page}`));
  }
  ...
  getSeasonEpisode(seriesId: number, seasonNumber: number): Observable<SeasonEpiosodes> {
    return this.limiter.limit(
      this.http.get<SeasonEpiosodes>(
        `${environment.apiUrl}tv/${seriesId}/season/` +
